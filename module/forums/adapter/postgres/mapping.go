...
func threadModelFromDomain(thread domain.Thread) ThreadModel {
	return ThreadModel{ID: orm.ID{ID: thread.ID}, ForumID: thread.ForumID, AuthorUserID: thread.AuthorUserID, OpenerPostID: thread.OpenerPostID, LatestPostID: thread.LatestPostID, LatestPostAuthorUserID: thread.LatestPostAuthorUserID, LatestPostAt: thread.LatestPostAt, Title: thread.Title, Slug: string(thread.Slug), Status: string(thread.Status), StickyState: string(thread.StickyState), StickyOrder: thread.StickyOrder, StickyUntil: thread.StickyUntil, LockedReason: thread.LockedReason, ReplyCount: thread.ReplyCount, VisibleReplyCount: thread.VisibleReplyCount, PostCount: thread.PostCount, VisiblePostCount: thread.VisiblePostCount, LikeCount: thread.LikeCount, ViewCount: thread.ViewCount, Version: thread.Version}
}

// threadFromModel maps persistence thread to domain.
func threadFromModel(model ThreadModel) domain.Thread {
	return domain.Thread{ID: model.ID.ID, ForumID: model.ForumID, AuthorUserID: model.AuthorUserID, OpenerPostID: model.OpenerPostID, LatestPostID: model.LatestPostID, LatestPostAuthorUserID: model.LatestPostAuthorUserID, LatestPostAt: model.LatestPostAt, Title: model.Title, Slug: domain.Slug(model.Slug), Status: domain.ThreadStatus(model.Status), StickyState: domain.StickyState(model.StickyState), StickyOrder: model.StickyOrder, StickyUntil: model.StickyUntil, LockedReason: model.LockedReason, ReplyCount: model.ReplyCount, VisibleReplyCount: model.VisibleReplyCount, PostCount: model.PostCount, VisiblePostCount: model.VisiblePostCount, LikeCount: model.LikeCount, ViewCount: model.ViewCount, Version: model.Version, CreatedAt: model.CreatedAt, UpdatedAt: model.UpdatedAt}
}

// postModelFromDomain maps post to persistence.
func postModelFromDomain(post domain.Post) PostModel {
	return PostModel{ID: orm.ID{ID: post.ID}, ThreadID: post.ThreadID, ForumID: post.ForumID, AuthorUserID: post.AuthorUserID, Sequence: post.Sequence, Status: string(post.Status), ContentFormat: string(post.ContentFormat), ContentDocumentJSON: string(post.ContentDocumentJSON), ContentText: post.ContentText, ContentChecksum: post.ContentChecksum, EditedAt: post.EditedAt, EditedByUserID: post.EditedByUserID, EditCount: post.EditCount, LikeCount: post.LikeCount, ReplyReferenceCount: post.ReplyReferenceCount, Version: post.Version}
}

// postFromModel maps persistence post to domain.
func postFromModel(model PostModel) domain.Post {
	return domain.Post{ID: model.ID.ID, ThreadID: model.ThreadID, ForumID: model.ForumID, AuthorUserID: model.AuthorUserID, Sequence: model.Sequence, Status: domain.PostStatus(model.Status), ContentFormat: domain.ContentFormat(model.ContentFormat), ContentDocumentJSON: json.RawMessage(model.ContentDocumentJSON), ContentText: model.ContentText, ContentChecksum: model.ContentChecksum, EditedAt: model.EditedAt, EditedByUserID: model.EditedByUserID, EditCount: model.EditCount, LikeCount: model.LikeCount, ReplyReferenceCount: model.ReplyReferenceCount, Version: model.Version, CreatedAt: model.CreatedAt, UpdatedAt: model.UpdatedAt}
}
...
func revisionModelFromDomain(revision domain.PostRevision) PostRevisionModel {
	return PostRevisionModel{ID: orm.ID{ID: revision.ID}, PostID: revision.PostID, EditedByUserID: revision.EditedByUserID, PreviousContentDocumentJSON: string(revision.PreviousContentDocumentJSON), PreviousContentText: revision.PreviousContentText, EditReason: revision.EditReason, CreatedAt: revision.CreatedAt}
}

// revisionFromModel maps persistence revision to domain.
func revisionFromModel(model PostRevisionModel) domain.PostRevision {
	return domain.PostRevision{ID: model.ID.ID, PostID: model.PostID, EditedByUserID: model.EditedByUserID, PreviousContentDocumentJSON: json.RawMessage(model.PreviousContentDocumentJSON), PreviousContentText: model.PreviousContentText, EditReason: model.EditReason, CreatedAt: model.CreatedAt}
}
...
func referenceModelFromDomain(reference domain.PostReference) PostReferenceModel {
	return PostReferenceModel{ID: orm.ID{ID: reference.ID}, SourcePostID: reference.SourcePostID, TargetPostID: reference.TargetPostID, TargetUserID: reference.TargetUserID, TargetAssetID: reference.TargetAssetID, ReferenceType: string(reference.ReferenceType), QuoteExcerpt: reference.QuoteExcerpt, LinkURL: reference.LinkURL, CreatedAt: reference.CreatedAt}
}
