...
func TestMigrateStatusReportsPendingMigration(t *testing.T) {
	...
	if !strings.Contains(output.String(), "pending=5") {
		t.Fatalf("output = %q, want pending=5", output.String())
	}
}
...
func TestMigrateCommandsApplyValidateAndReset(t *testing.T) {
	...
	if !strings.Contains(output, "applied=5 pending=0") {
		t.Fatalf("up output = %q, want applied=5 pending=0", output)
	}
	...
	if !strings.Contains(output, "applied=5 pending=0") {
		t.Fatalf("validate output = %q, want applied=5 pending=0", output)
	}
	...
	if !strings.Contains(output, "applied=0 pending=5") {
		t.Fatalf("reset output = %q, want applied=0 pending=5", output)
	}
}
