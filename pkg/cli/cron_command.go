...
func newCronRunCommand(activeLogger **zap.Logger, deps commandDeps) *cobra.Command {
	return &cobra.Command{
		Use:           "run",
		Short:         "Run due cron jobs until stopped",
		SilenceUsage:  true,
		SilenceErrors: true,
...
func newCronRunOnceCommand(activeLogger **zap.Logger, deps commandDeps) *cobra.Command {
	return &cobra.Command{
		Use:           "run-once",
		Short:         "Run one due cron job",
		SilenceUsage:  true,
		SilenceErrors: true,
		RunE: func(cmd *cobra.Command, _ []string) error {
			return runCronSummary(
				cmd.Context(),
				activeLogger,
				deps,
				cmd,
				func(ctx context.Context, service cronapp.Service) (cronapp.RunSummary, error) {
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			return runCronAction(cmd.Context(), activeLogger, deps, func(ctx context.Context, service cronapp.Service) error {
				...
				for _, definition := range result.Items {
					fmt.Fprintf(
						cmd.OutOrStdout(),
						"job key=%s enabled=%t next_run_at=%v last_status=%s\n",
						definition.Key,
						definition.Enabled,
...
func runCronSummary(
	...
	deps commandDeps,
	cmd *cobra.Command,
	action func(context.Context, cronapp.Service) (cronapp.RunSummary, error),
) error {
	return runCronAction(ctx, activeLogger, deps, func(ctx context.Context, service cronapp.Service) error {
		...
		fmt.Fprintf(
			cmd.OutOrStdout(),
			"run_id=%s job_key=%s processed=%d failed=%t\n",
			result.RunID,
			result.JobKey,
...
func runCronAction(
	...
	activeLogger **zap.Logger,
	deps commandDeps,
	action func(context.Context, cronapp.Service) error,
) error {
